...
from database.repository import update_deal_status, get_deal_by_id, get_user_language, get_username, \
    add_referral_revenue, revenue_update, update_last_activity, get_deal_by_hex, is_user_vip
...
async def finalize_deal(callback: CallbackQuery, deal):
    ...
    if success:
        ...
        if buyer.invited_by is not None and seller.invited_by is not None:
            if is_user_vip(deal.buyer_id) and is_user_vip(deal.seller_id):
            ...
            elif is_user_vip(deal.buyer_id) and (not is_user_vip(deal.seller_id)):
            ...
            elif (not is_user_vip(deal.buyer_id)) and is_user_vip(deal.seller_id):
                ref = ((deal.comission_price - fee)*Config.REFERAL_COMMISSION_VIP)/2
                ref_vip = ((deal.comission_price - fee)*Config.REFERAL_COMMISSION_VIP)/2
                our_revenue = deal.comission_price -fee- (ref + ref_vip)
            ...
            if is_user_vip(deal.buyer_id_id):
